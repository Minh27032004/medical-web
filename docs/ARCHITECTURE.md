...
| Medicines | CRUD `/doctor/medicines` (+units), `GET ?q=` (không dấu, xếp theo độ liên quan), `GET /{id}` (trang sửa), `POST /{id}/adjust-stock` {entries:[{unitName,qty}], reason}, `GET /low-stock`, `GET /suggest?q=` (ưu tiên template) | ROLE_DOCTOR |
